# SSU 1.9.0-dev3.10 runtime test checklist

Build basis: `1.9.0-dev3.9.1`  
Target: Minecraft 26.2 / NeoForge 26.2.0.7-beta / Java 25  
Network protocol: `95`  
Changed persistence: Server Operations schema `2`
...
As a normal player:
- open **Support**;
- cycle every fixed category (Help, Bug, Player report, Suggestion, Appeal, Other);
- for Player report, open the player picker, search/page through known players and select one; confirm self-reporting is refused server-side;
- open **Edit message**, enter multiline text and apply several colours + bold/italic/underline/strikethrough; save and create the ticket;
- confirm the created ticket appears with its formatted first message and persists after restart;
- confirm the fourth simultaneous OPEN/ASSIGNED ticket is refused;
- create enough tickets to exercise list Previous/Next paging;
- open a ticket, add multiple rich-text replies and verify each becomes a separate chronological thread message;
- confirm a staff reply causes a `●` unread indicator plus an `[SSU Support]` notification while the player is online;
- open the ticket and confirm the unread indicator clears;
- reply to a RESOLVED ticket and confirm it automatically returns to OPEN/ASSIGNED;
- close a ticket and confirm further replies are refused.

As staff/admin:
- open **Server operations → Reports**;
- confirm new/replied tickets show `●` unread state and online staff receive a support notification;
- test the ALL/OPEN/ASSIGNED/RESOLVED/CLOSED status filter and list paging;
- open a ticket and confirm reported-player metadata is separate from the conversation text;
- use **Edit rich reply** and send several replies; confirm the old single `staffNote` overwrite behavior is gone;
- test Assign me, Resolve, Reopen and Close;
- confirm the first staff reply auto-assigns an otherwise unassigned OPEN ticket;
- create enough replies to exercise Older/Newer conversation paging;
- verify ticket actions remain in Staff Audit and ticket/reply notifications are only sent to appropriate online staff/player recipients.

Migration test:
- start once with a copy of a dev3.9.x `server_operations/state.json` containing old `message` / `staffNote` ticket data;
- confirm schema 2 loads it without loss and converts those values into player/staff thread entries only once.

Restart and confirm categories, report target, status, assignee, unread state and complete stored thread history persist.
