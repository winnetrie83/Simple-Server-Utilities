...
public final class NpcQuestAccessPromptScreen extends Screen {
    ...
    private void returnThen(Runnable action){if(minecraft!=null)minecraft.setScreenAndShow(parent);action.run();}
    @Override public void onClose(){if(minecraft!=null)minecraft.setScreenAndShow(parent);}
    @Override public void extractRenderState(GuiGraphicsExtractor g,int mx,int my,float pt){int x=left(),y=top();SsuGuiScale.fullscreenDim(g, this, 0xA5000000);g.fill(x,y,x+W,y+H,0xF0161D25);g.outline(x,y,W,H,0xFF586978);g.text(font,"NPC quest access is currently disabled",x+16,y+16,0xFFFFC857,true);g.text(font,"Choose how players should access quests before this link is saved.",x+16,y+38,0xFFAAB5BE,false);g.text(font,"You can change this later in SSU Settings.",x+16,y+50,0xFFAAB5BE,false);super.extractRenderState(g,mx,my,pt);}private int left(){return(width-W)/2;}private int top(){return(height-H)/2;}
}
