...
public final class AchievementRewardInventoryPickerScreen extends Screen {
    ...
    @Override public boolean mouseClicked(MouseButtonEvent e,boolean dbl){if(e.buttonInfo().button()==0){int slot=slotAt((int)e.x(),(int)e.y());if(slot>=0&&!inventory(slot).isEmpty()){selected=slot;rebuildWidgets();return true;}}return super.mouseClicked(e,dbl);}
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor g,int mx,int my,float pt){int x=left(),y=top();g.fill(0,0,width,height,0xA5000000);g.fill(x,y,x+W,y+H,PANEL);g.outline(x,y,W,H,BORDER);g.text(font,"Choose reward item",x+16,y+14,TEXT,true);g.text(font,"Select a stack from your inventory. Nothing is consumed.",x+16,y+32,MUTED,false);int sx=x+54,sy=y+52;for(int row=0;row<3;row++)for(int col=0;col<9;col++)draw(g,9+row*9+col,sx+col*18,sy+row*18,mx,my);for(int col=0;col<9;col++)draw(g,col,sx+col*18,sy+60,mx,my);super.extractRenderState(g,mx,my,pt);}
    private void draw(GuiGraphicsExtractor g,int slot,int x,int y,int mx,int my){boolean hover=SsuGuiGeometry.inside(mx,my,x,y,18,18);g.fill(x,y,x+18,y+18,0xE00A0F14);g.outline(x,y,18,18,slot==selected?SELECTED:(hover?GOOD:BORDER));ItemStack s=inventory(slot);if(!s.isEmpty()){g.item(s,x+1,y+1);g.itemDecorations(font,s,x+1,y+1);if(hover)g.setTooltipForNextFrame(font,s,mx,my);}}
