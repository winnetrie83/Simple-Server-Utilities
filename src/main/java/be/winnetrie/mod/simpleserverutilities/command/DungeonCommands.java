...
public final class DungeonCommands {
    ...
    private static int open(CommandSourceStack source){
        if (!requireModule(source)) return 0;ServerPlayer player=player(source);if(player==null)return 0;SimpleServerUtilities.DUNGEONS.open(player);return 1;}
    private static int join(CommandSourceStack source,String id){
        if (!requireModule(source)) return 0;ServerPlayer player=player(source);if(player==null)return 0;try{player.sendSystemMessage(Component.literal(SimpleServerUtilities.DUNGEONS.joinQueue(player,id)));return 1;}catch(RuntimeException e){source.sendFailure(Component.literal(message(e)));return 0;}}
    private static int leave(CommandSourceStack source){
        if (!requireModule(source)) return 0;ServerPlayer player=player(source);if(player==null)return 0;player.sendSystemMessage(Component.literal(SimpleServerUtilities.DUNGEONS.leave(player,true)));return 1;}
    private static int status(CommandSourceStack source){
        if (!requireModule(source)) return 0;if(!canAdmin(source)){source.sendFailure(Component.literal("Dungeon administrator permission is required."));return 0;}var s=SimpleServerUtilities.DUNGEONS.snapshot();source.sendSystemMessage(Component.literal("SSU Dungeons: definitions="+s.definitions()+", queued="+s.queuedPlayers()+", runs="+s.runs()+", reserved="+s.reservedArenas()+", resetting="+s.resettingArenas()+", blocked="+s.blockedArenas()+", recoveries="+s.pendingRecoveries()));return 1;}
    ...
    private static int complete(CommandSourceStack source,String id,boolean success){
    ...
    private static int advance(CommandSourceStack source){
    ...
    private static int release(CommandSourceStack source,String target){
        if (!requireModule(source)) return 0;if(!canAdmin(source)){source.sendFailure(Component.literal("Dungeon administrator permission is required."));return 0;}source.sendSystemMessage(Component.literal(SimpleServerUtilities.DUNGEONS.releaseBlockedArena(target)));return 1;}
    ...
    private static boolean requireModule(CommandSourceStack source) {
        if (be.winnetrie.mod.simpleserverutilities.core.module.SsuModuleAccess.active("dungeons")) return true;
        source.sendFailure(Component.literal("Dungeons is disabled or blocked by a required dependency."));
        return false;
    }
}
